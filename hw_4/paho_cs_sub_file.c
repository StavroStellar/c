...
struct pubsub_opts opts =
{
	...
	MQTTVERSION_DEFAULT, "/node-red/temp", "paho-cs-sub", 0, 0, "", "", "", "1883", NULL, 10, /* MQTT options */
...
{ 
	if (argc != 6) {
        printf("Usage: %s <IP> <login> <password> <filename>  <port>\n", argv[0]);
        return 1;
    }
	char *ip = argv[1];
    char *login = argv[2];
...
//				logProperties(&message->properties);
   ...
   if (fprintf(file, data) < 0) {
        ...
        return 1;
			...
			fflush(file); // сброс буфера для уверенности в записи данных в файл
